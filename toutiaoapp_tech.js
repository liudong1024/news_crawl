...
//http://is.snssdk.com/2/article/information/v21/ 详情
    ...
    function next1() {
        ...
        var url = "http://is.snssdk.com/api/news/feed/v62/?refer=1&count=20&loc_mode=4&device_id=34960436458&iid=13136511752&category=news_tech&min_behot_time=1507540706&last_refresh_sub_entrance_interval=1507541438";
        ...
        request.get({url: url, headers: headers}, function (err, httpResponse, body) {
            try {
                ...
                if (body.indexOf("article-content") >= 0 && body.indexOf("article-title") >= 0) {
                    ...
                    sql = "insert ignore into tech_2017 (__url, article_title, article_author, article_categories, url,article_publish_time,article_content,__time,article_avatar,article_topics,article_thumbnail,comments_count) values ('" + url + "','" + article_title + "','" + article_source + "','" + "科技" + "','" + url + "','" + article_publish_time*1000 + "','" + content + "','" + currentTime*1000 + "','" + article_avatar + "','" + article_topics + "','" + article_thumbnail + "','" + comments_count + "')";
                    console.log(sql);
                    pool.query(sql);
                    // console.log(content);
